...
public class DatabaseExceptionHandler {
    ...
    public String handleExpenditureQueryError(Exception e) {
        logger.error("Ошибка при выполнении запроса расходов");
        if (e instanceof RepositoryException) {
            return "Ошибка базы данных при получении расходов: " + e.getMessage();
        }
        return "Ошибка при выполнении запроса расходов: " + e.getMessage();
    }
    ...
    public String handleMathSchoolsQueryError(Exception e, int minStudents, int maxStudents) {
        ...
        if (e instanceof RepositoryException) {
            return String.format("Ошибка базы данных для диапазона %d-%d: %s", minStudents, maxStudents, e.getMessage());
        }
        return String.format("Ошибка для диапазона %d-%d: %s", minStudents, maxStudents, e.getMessage());
    }
